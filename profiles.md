1)COMENTARIOS:
a)¿se loguean?
-si se loguean ya que tiene que utilizar su id.
b)¿A que tablas tiene acceso?
-tiene acceso a la tabla de comentarios y recomendaciones
c)¿que infirmacion puedo:crear,actualizar,leer,y eliminar?
-crear,leer,utilizar,actualizar,eliminar

2)RECOMENDACIONES:
a)¿se loguean?
-si de loguean ya que utiliza su id
b)¿A que tablas tiene acceso?
-tiene acceso a la tabla de recomendaciones y comentarios
c)¿que infirmacion puedo:crear,actualizar,leer,y eliminar?
-crear,leer,utilizar,actualizar,eliminar

3)TRABAJADORES:
a)¿se loguean?
-Si ya que debe poner su id,su nombre y su funcion
b)¿A que tablas tiene acceso?
-tiene acceso a todas las tablas
c)¿que infirmacion puedo:crear,actualizar,leer,y eliminar?
-crear,leer,utilizar,actualizar,eliminar
